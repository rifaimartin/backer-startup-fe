    ...
    <div class="flex ml-auto" v-else>
      <div class="dropdown inline-block relative z-10">
        <button class="bg-white text-gray-700 font-semibold py-4 px-6 rounded inline-flex items-center">
          <img :src="$axios.defaults.baseURL + '/' + $store.state.auth.user.image_url" alt=""
          ...
          <ul class="dropdown-menu absolute hidden text-gery-700 pt-1 shadow w-full -mt-2">
            <li>
              <nuxt-link to="/dashboard"
                class="bg-white hover:bg-gray-100 hover:text-orange-500 py-2 px-4 block whitespace-no-wrap">My Dashboard
              </nuxt-link>
            </li>
            <li>
              <nuxt-link to="/dashboard"
                class="bg-white hover:bg-gray-100 hover:text-orange-500 py-2 px-4 block whitespace-no-wrap">Account Settings
              </nuxt-link>
            </li>
             <li>
              <a @click="logout()"
                class="cursor-pointer rounded-b bg-white hover:bg-gray-100 hover:text-orange-500 py-2 px-4 block whitespace-no-wrap">Logout
              </a>
            </li>
          </ul>
      </div>
